...
export const GET = async (req: NextRequest, context: { params: { id: string }}) => {
    try {
        ...
        const { data: site, error: site_error } = await supabase.from("sites").select("*").limit(1).eq("short_id", short_id)
        ...
        const { data, error } = await supabase.from("tokens").select("partial_key, created_at, updated_at, id, expires, name, last_used").eq("user_id", user.id).eq("site_id", site[0].id)
